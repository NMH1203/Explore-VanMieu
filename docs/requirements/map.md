...
frontend/src/
├── components/map-view/
│   ├── InteractiveMap.jsx          # Bản đồ Leaflet, marker HTML, zoom/pan tự động
│   └── LocationSidebar.jsx         # Danh sách địa điểm đồng bộ với bản đồ
├── data/
│   └── mapLocations.js             # Dữ liệu dùng chung: 10 công trình, GPS và bounding box Văn Miếu
└── pages/map/
    ├── MapPage.jsx                 # Component chính: Điều phối layout, state lựa chọn địa điểm
    └── map.css                     # CSS giao diện bản đồ và responsive
```

### Chi tiết vai trò từng file:
1. **`frontend/src/data/mapLocations.js`**:
...
2. **`frontend/src/components/map-view/InteractiveMap.jsx`**:
...
3. **`frontend/src/components/map-view/LocationSidebar.jsx`**:
