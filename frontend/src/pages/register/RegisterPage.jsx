  ...
  return <section className="account-auth">
    ...
    <div className="account-auth__content">
      ...
      <div className="account-auth__tabs" role="tablist">
        ...
        <button className={signup ? 'active' : ''} type="button" onClick={() => setMode('signup')}>Sign up</button></div>
      <form onSubmit={(event) => event.preventDefault()}>
        {signup && <input aria-label="Full name" type="text" placeholder="Full name" autoComplete="name" required />}
        <input aria-label="Email" type="text" placeholder="Email" autoComplete="Email" required />
      ...
      <p className="account-auth__notice">
        <CircleHelp aria-hidden="true" />Browsing the Gallery, Categories and Digital Experience is always free — you only need an account to scan QR codes and collect heritage stamps.</p>
    </div></div>
  </section>
}
